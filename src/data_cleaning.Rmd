...
emp_length_loan_counts %>%
  ...
  scale_fill_manual(values = c("red", "green"), labels = c("Bad Loan (0)", "Good Loan (1)")) +
...
loan_counts_per_year <- combined_data %>%
  group_by(issue_d, loan_status_bin) %>%
  summarize(loan_count = n()) %>%
  ungroup() %>%
  group_by(issue_d) %>%
  mutate(total_loans = sum(loan_count),
...
loan_counts_per_year %>%
  ggplot(aes(x = issue_d, y = percentage_loans, fill = as.factor(loan_status_bin))) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(
