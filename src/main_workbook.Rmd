...
### Creating the tree
```{r}
decision_tree <- rpart(will_default ~ ., 
                       data = train_data, 
                       method = "class", 
...
```{r}
tree_predictions <- predict(decision_tree, test_data, type = "prob")[,2]
```
...
# clusterExport(cluster, c('train_data', 'params_bounds', 'dtrain'))
# clusterEvalQ(cluster, expr = {library(xgboost)})


# Opt for 5 epochs of tuning
params_opt = bayesOpt(FUN = score, bounds = params_bounds, initPoints = 8, iters.n = 5)
...
calculate_precision_recall <- function(probs, labels, model_name) {
  ...
  #   # Replace NaN precision values with zero
  #   pr_data$precision[is.nan(pr_data$precision)] <- 1
  # }
...
models <- list(
  ...
  list(name = "Decision Tree", predictions = predict(decision_tree, test_data, type = "class")),
